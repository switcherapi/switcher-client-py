...
from tests.helpers import (
    given_check_snapshot_version,
    given_context,
    given_auth,
    given_check_criteria,
    given_check_criteria_exception,
    given_check_health,
    given_resolve_snapshot
)
from tests.test_client_load_snapshot_remote import load_snapshot_fixture
...
def test_silent_mode_for_check_criteria_in_memory_snapshot(httpx_mock):
    """ Should use the silent mode when the remote API is not available for check criteria using the in-memory snapshot """

    options = ContextOptions(**vars(context_options))
    options.snapshot_location = None
    options.silent_mode = '1s'

    # given
    given_auth(httpx_mock)
    given_check_snapshot_version(httpx_mock, version=0, status=False)
    given_resolve_snapshot(httpx_mock, data=load_snapshot_fixture('tests/snapshots/default.json'))
    given_check_criteria(httpx_mock, key='FF2FOR2022', response={'error': 'Too many requests'}, status=429)
    given_check_health(httpx_mock, status=500)
    given_context(options=options)
...
def test_silent_mode_for_check_criteria_timeout(httpx_mock):
    """ Should use silent mode when the remote criteria endpoint times out """
    ...
    # given
    given_auth(httpx_mock)
    given_check_criteria_exception(httpx_mock, httpx.ConnectTimeout('timed out'), key='FF2FOR2022')
    given_context(options=options)
